...
public class Parser {
	...
	public List<String> GetPackages(CompilationUnit cu,String pakage){
        ...
        for( ImportDeclaration package1 : node_list)
        {
        	try
        	{
        		if(package1.getNameAsString().indexOf(pakage,0)!=-1)// !=-1 per solo JDK
        		{
	...
	public List<String> GetMethods(CompilationUnit cu){
        ...
        for( MethodDeclaration method : node_list)
        {
        	try{      		
        		...
        		methods.add(method.getBody().toString());
